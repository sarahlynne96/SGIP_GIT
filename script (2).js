...
<body>
  ...
  <script>
    ...
    /***** county AMI data *****/ // fileciteturn2file0
    ...
    /***** SGIP rate catalogue (unchanged) *****/ // fileciteturn2file0
    ...
    function catalog(){ return [
      ...
      {key:'gen',name:'General Rate',badge:'General',
        elig:u=>true,rate:`${rateFmt(RATES.gen)}/Wh`,
        calc:u=>u.stWh*RATES.gen}
    ]; }
    ...
    function restoreDraft(){
      ...
      const obj=JSON.parse(d); Object.entries(obj).forEach(([k,v])=>{ const el=$(`[name="${k}"]`); if(el) el.value=v; });
    }
    ...
    window.addEventListener('DOMContentLoaded',()=>{
      ...
      $('#calcForm').addEventListener('submit', e=>{
        ...
        const stWh=parseFloat(data.storageKWh)||0, solW=(parseFloat(data.solarKW)||0)*1000;
        ...
        const incEl=$('#incomeResult'), resEl=$('#result'); incEl.style.display=resEl.style.display='block';
        ...
        const tracks=catalog().filter(t=>t.elig(user));
        resEl.innerHTML='<h3>Estimated SGIP Rebates</h3>';
        tracks.forEach(t=>resEl.innerHTML+=`<div><span class="badge">${t.badge}</span>${t.name}: <strong>${money(t.calc(user))}</strong></div>`);
      });
    });
  </script>
</body>
